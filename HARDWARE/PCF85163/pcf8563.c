...
u8 PCF8563_Get_Week(u16 year, u8 month, u8 day)
{
	u16 temp2;
	u8 yearH, yearL;
...
void PCF8563_Set_Times(unsigned char PCF_Format,\
	                   ...
	                   unsigned char Year, unsigned char Month, unsigned char Date,\
                       unsigned char Hour, unsigned char Minute,unsigned char sec)
{
	...
	Week = PCF8563_Get_Week(Year + 2000, Month, Date);
	if(Week == 7) Week = 0;
	...
	if (PCF_Format == PCF_Format_BIN)
	{
		...
		Time_InitStructure.RTC_Seconds  = RTC_BinToBcd2(sec);
	}
...
void PCF8563_SetAlarm(unsigned char PCF_Format, _PCF8563_Alarm_Typedef* PCF_DataStruct)
{
	...
	if (PCF_DataStruct->RTC_AlarmWeekDays == 7) PCF_DataStruct->RTC_AlarmWeekDays = 0;
...
u8 PCF8563_Init(void)
{
	IIC_GPIO_Init();
	
	if(PCF8563_Check() == 0){   //检测器件是否存在
			PCF8563_Start();
			PCF8563_SetMode(PCF_Mode_Normal);
			PCF8563_SetPowerReset(PCF8563_PowerResetDisable);
		}
		else{	//此时不存在
			return 1;
		}
	...
	PCF8563_Stop();
	PCF8563_Set_Times(PCF_Format_BIN, PCF_Century_20xx, 23, 4, 7, 9, 29, 30);
	PCF8563_Start();
		return 0;
}

//判断是否产生闹钟中断
u8 PCF8563_isAlarm(void)
{
	u8 Alarm_Interrupt = 0;
	//
	//读取控制/状态寄存器2值
	//
	Alarm_Interrupt = PCF8563_Read_Byte(PCF8563_Address_Control_Status_2);
	
	if(Alarm_Interrupt & 0x04)
	{
		return 1;
	}
	else{
